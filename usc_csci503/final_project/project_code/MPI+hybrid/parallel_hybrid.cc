...
extern double * parallel_hybrid_main(int N, double * array, int threadNum/*, double* overhead,int numPro*/);
//double overhead = 0.0;
...
int main(int argc, char *argv[])
{
	...
	if(rank==0){
		...
		time=read_timer();
       		parallelArray=parallel_hybrid_main(N,parallelArray, threadNum/*,&overhead*/);
		...
		parallel_hybrid_main(N,parallelArray, threadNum/*,&overhead*/);
		//printf("first here,%d\n",rank);
	}
    }
    MPI_Finalize();
