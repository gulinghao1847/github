...
extern double * parallel_main(int N, double * array/*, double* overhead,int numPro*/);
//double overhead = 0.0;
...
int main(int argc, char *argv[])
{
    ...
    double initTime, randomGeneTime, overhead;
    elapse1=0; 
    elapse2=0;
    overhead=0;
    time=read_timer();
    ...
    initTime=read_timer()-time;
    int N = atoi(argv[1]);
    numPro=atoi(argv[2]);
    if(rank==0){
	time_2=read_timer();
	A=MakeAIntArray(N);//
	randomGeneTime=read_timer()-time_2; 
	...
	if(rank==0){
		...
		randomGeneTime=read_timer()-time_2;
		time=read_timer();
       		parallelArray=parallel_main(N,parallelArray/*,&overhead*/);
		elapse2+=read_timer()-time;
		//print(N,parallelArray);
		printf("parallel time:%f\n",elapse2);
		printf("overhead of init: %f\n", initTime);
		printf("speedup: %f\n", elapse1/elapse2);
		printf("overhead of randomGeneTime: %f\n", randomGeneTime);
	}else{
		parallel_main(N,parallelArray/*,&overhead*/);
		//printf("first here,%d\n",rank);
	}
    }
    MPI_Finalize();
